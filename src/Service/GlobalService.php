...
class GlobalService{
    ...
    public function cronOcrIa($entreprise = null){
        ...
        if(!is_null($entreprise)){
            $documents = $this->em->getRepository(EmailDocumentPreview::class)->findBy(['execute'=>false, 'extension'=>'pdf', 'entreprise'=>$entreprise->getId(), 'sender'=>"mdistri@gmail.com"], ['id'=>"ASC"], 10);
        }
        else{
            $documents = $this->em->getRepository(EmailDocumentPreview::class)->findBy(['execute'=>false, 'extension'=>'pdf', 'sender'=>"mdistri@gmail.com"], ['id'=>"ASC"], 10);
        }
